...
play.addEventListener('click', function () {
	...
	header.innerHTML = '<h1>Formulir Diagnosis Malnutrisi Anak Balita</h1>';
	getDiagnose();
});
...
function modalBox() {
	...
	divContent.innerHTML =
		'<p>Isi jawaban terlebih dahulu sebelum lanjut ke pertanyaan selanjutnya.</p>';
	...
	fetch('./data/data.json')
		...
		.then(function (data) {
			...
			next.addEventListener('click', function () {
				...
				const idJawaban = document.querySelectorAll(
					'#formulir div#Jawaban div input'
				);
				...
				if (!flagEmpty) {
					modalBox();
					document
						.querySelector('.modal-close input')
...
function showQuestion(data, index, jawab) {
	...
	for (let j = 0; j < data[1].isiKodeUtama[index].jawaban.length; j++) {
		...
		divJawaban.setAttribute(
			'id',
			`Jawaban${data[1].isiKodeUtama[index].kode}-${j}`
		);
...
function showResult(data, array) {
	const header = document.getElementById('header');
	header.innerHTML = '<h1>Hasil Diagnosis Malnutrisi Anak Balita</h1>';
	...
	divHasilUtamaAnalisis.innerHTML =
		'<p>Berdasarkan formulir yang telah diisi, maka hasil analisis yang didapat adalah</p>';
	...
	buttonPrint.setAttribute('value', 'Cetak Hasil');
	buttonPrint.setAttribute('onclick', 'printResult()');
	buttonDiv.append(buttonPrint, buttonBack);
	...
	if (
		...
		if (
			...
			divHasilUtamaAnalisis.innerHTML +=
				'<h2>' +
				data[0].isiKodeUtama[0].isiKode +
				'</h2>' +
				'<p>' +
				data[0].isiKodeUtama[0].deskripsiKode +
				'</p>';
			...
			divHasilUtamaAnalisis.innerHTML +=
				'<h2>' +
				data[0].isiKodeUtama[1].isiKode +
				'</h2>' +
				'<p>' +
				data[0].isiKodeUtama[1].deskripsiKode +
				'</p>';
			for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
				for (let j = 0; j < array.length; j++) {
					if (data[1].isiKodeUtama[i].kode === array[j][0]) {
			...
			divHasilUtamaAnalisis.innerHTML +=
				'<h2>' +
				data[0].isiKodeUtama[2].isiKode +
				'</h2>' +
				'<p>' +
				data[0].isiKodeUtama[2].deskripsiKode +
				'</p>';
			for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
				for (let j = 0; j < array.length; j++) {
					if (data[1].isiKodeUtama[i].kode === array[j][0]) {
		...
		for (let i = 0; i < data[2].isiKodeUtama[0].isiKode.length; i++) {
			divSolusiAnalisis.innerHTML +=
				'<p>' + data[2].isiKodeUtama[0].isiKode[i] + '</p>';
		}
	} else if (
		array[0][0] == 'GG01' &&
		array[0][1] == 1 &&
		...
		divHasilUtamaAnalisis.innerHTML +=
			'<h2>' +
			data[0].isiKodeUtama[3].isiKode +
			'</h2>' +
			'<p>' +
			data[0].isiKodeUtama[3].deskripsiKode +
			'</->';
		for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
			for (let j = 0; j < array.length; j++) {
				if (data[1].isiKodeUtama[i].kode === array[j][0]) {
		...
		for (let i = 0; i < data[2].isiKodeUtama[1].isiKode.length; i++) {
			divSolusiAnalisis.innerHTML +=
				'<p>' + data[2].isiKodeUtama[1].isiKode[i] + '</p>';
		}
	} else if (
		array[0][0] == 'GG01' &&
		array[0][1] == 2 &&
		...
		divHasilUtamaAnalisis.innerHTML +=
			'<h2>' +
			data[0].isiKodeUtama[4].isiKode +
			'</h2>' +
			'<p>' +
			data[0].isiKodeUtama[4].deskripsiKode +
			'</p>';
		for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
			for (let j = 0; j < array.length; j++) {
				if (data[1].isiKodeUtama[i].kode === array[j][0]) {
		...
		for (let i = 0; i < data[2].isiKodeUtama[2].isiKode.length; i++) {
			divSolusiAnalisis.innerHTML +=
				'<p>' + data[2].isiKodeUtama[2].isiKode[i] + '</p>';
		}
	} else if (
		array[0][0] == 'GG01' &&
		array[0][1] == 3 &&
		...
		divHasilUtamaAnalisis.innerHTML +=
			'<h2>' +
			data[0].isiKodeUtama[5].isiKode +
			'</h2>' +
			'<p>' +
			data[0].isiKodeUtama[5].deskripsiKode +
			'</p>';
		for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
			for (let j = 0; j < array.length; j++) {
				if (data[1].isiKodeUtama[i].kode === array[j][0]) {
		...
		for (let i = 0; i < data[2].isiKodeUtama[3].isiKode.length; i++) {
			divSolusiAnalisis.innerHTML +=
				'<p>' + data[2].isiKodeUtama[3].isiKode[i] + '</p>';
		}
	} else {
		divHasilUtamaAnalisis.innerHTML +=
			'<h2>' +
			data[0].isiKodeUtama[6].isiKode +
			'</h2>' +
			'<p>' +
			data[0].isiKodeUtama[6].deskripsiKode +
			'</p>';
		for (let i = 0; i < data[1].isiKodeUtama.length; i++) {
			for (let j = 0; j < array.length; j++) {
				if (data[1].isiKodeUtama[i].kode === array[j][0]) {
		...
		divSolusiAnalisis.innerHTML +=
			'<p>Tidak ada solusi yang dapat diambil.</p>';
	}
	divUtama.append(
		divHasilUtamaAnalisis,
		divPertanyaanAnalisis,
		divSolusiAnalisis
	);
}
